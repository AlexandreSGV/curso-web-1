...
|---|---|---|
| arquivo, `artisan` ou `index.php` não encontrado | terminal aberto na pasta errada | No CMD, execute `cd` para conferir a pasta atual e `dir` para listar os arquivos. No Ubuntu, use `pwd` e `ls`. Entre na pasta correta antes de repetir o comando. |
| comando não reconhecido ou `command not found` | programa ausente ou pasta fora do `PATH` | Feche e abra o terminal. No Windows, use `where NOME`; no Ubuntu, use `command -v NOME`. Depois consulte o guia de ambiente da disciplina. |
...
| Ubuntu/WSL não abre ou não aparece | distribuição não instalada ou WSL não iniciado | No CMD, execute `wsl -l -v`. Depois abra **Ubuntu 24.04** pelo menu Iniciar. |
...
3. o resultado de `cd` e `dir` no Windows, ou de `pwd` e `ls` no Ubuntu/WSL;
4. qual terminal está usando: CMD ou Ubuntu/WSL;
